...
public class AddContestQuestionsController {
    ...
    public void initData(int contestId, String contestName) {
        ...
        this.currentContestName = contestName;
        contestTitleLabel.setText("Add Questions to: " + contestName);
    }
    ...
    private void handleAddQuestion() {
        ...
        int newQuestionId = DatabaseConnector.addContestQuestion(currentContestId, questionText, type, marks);

        if (newQuestionId != -1) {
            if ("MCQ".equals(type)) {
                DatabaseConnector.addContestMcqOption(newQuestionId, option1Field.getText(), option1Radio.isSelected());
                DatabaseConnector.addContestMcqOption(newQuestionId, option2Field.getText(), option2Radio.isSelected());
                DatabaseConnector.addContestMcqOption(newQuestionId, option3Field.getText(), option3Radio.isSelected());
                DatabaseConnector.addContestMcqOption(newQuestionId, option4Field.getText(), option4Radio.isSelected());
            }
    ...
    private void handleFinish() {
        try {
            ...
            e.printStackTrace();
            statusLabel.setText("Error: Could not return to contest menu.");
            statusLabel.setTextFill(javafx.scene.paint.Color.RED);
        }
    }
}